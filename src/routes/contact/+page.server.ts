...
// Note in this import 'ZodError' is not needed - it will be inferred in the proper place
import { z } from 'zod';
// These imports aren't required to make this work
...
// The load function isn't required - no data is being loaded
...
export const actions: Actions = {
  submit: async ({ request }) => {
    ...
    //
    // By using 'safeParse' instead of just 'parse', Zod will not throw an error if the vallidation
    ...
    if (!parsedContactForm.success) {
      ...
      const errors = parsedContactForm.error.errors.map(err => {
        return {
          // Return whatever shape you need. This is just one option. 
          //
      ...
      // Also note - SK should be smart enough to preserve form data that was entered.
      throw fail(400, { error: true, errors })
    }

    // If you got here, you have valid form data. Now you can extract it. 
    // We'll access it below. 

    // URL construction is not changed
